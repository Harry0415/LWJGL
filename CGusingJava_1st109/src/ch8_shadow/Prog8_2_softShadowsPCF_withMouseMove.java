...
public class Prog8_2_softShadowsPCF_withMouseMove extends JFrame implements GLEventListener, MouseMotionListener
{	private GLCanvas myCanvas;
	...
	private Vector3f userLightPos = new Vector3f();
	private float[] lightPos = new float[3];
	private Vector3f origin = new Vector3f(0.0f, 0.0f, 0.0f);
	private Vector3f up = new Vector3f(0.0f, 1.0f, 0.0f);
	
	// Mouse Move Light
	private Vector3f initailLightLoc = new Vector3f(-3.0f, 3.0f, 2.0f);
	int lastX = 0, lastY = 0;
	boolean movedLeft = false, movedRight = false;
	float lightInc = 0.2f;
	// Mouse Move Light
	
	public Prog8_2_softShadowsPCF_withMouseMove()
	{	setTitle("Chapter8 - program 2 (With Mouse Move Light)");
		setSize(800, 800);
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		...
		// Mouse Move Light
//		currentLightPos.set(lightLoc); // Origin
		currentLightPos.set(userLightPos);
		// Mouse Move Light
...
//		currentLightPos.set(LightLoc); //Origin
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		renderingProgram1 = Utils.createShaderProgram("code/vertShader4Prog8_2_1.glsl", "code/fragShader4Prog8_2_1.glsl");
		renderingProgram2 = Utils.createShaderProgram("code/vertShader4Prog8_2_2.glsl", "code/fragShader4Prog8_2_2.glsl");
		...
		// Mouse Move Light
		currentLightPos.set(initailLightLoc);
		userLightPos.set(initailLightLoc);
		// Mouse Move Light
	}
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		...
		// pyramid definition
		ImportedModel pyramid = new ImportedModel("/models/pyr.obj");
	...
	public static void main(String[] args) { new Prog8_2_softShadowsPCF_withMouseMove(); }
	...
	public void mouseMoved(MouseEvent e) 
	{
		int x = e.getX();
		int y = e.getY();
		if (x < lastX) movedLeft=true; else movedLeft=false;
		if (x > lastX) movedRight=true; else movedRight=false;
		lastX = x;
		lastY = y;
		
		if (movedLeft) userLightPos.set(new Vector3f(
				userLightPos.x()-lightInc,
				userLightPos.y(),
				userLightPos.z()));
		if (movedRight) userLightPos.set(new Vector3f(
				userLightPos.x()+lightInc,
				userLightPos.y(),
				userLightPos.z()));
	}
